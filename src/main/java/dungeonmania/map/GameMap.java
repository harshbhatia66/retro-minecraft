...
public class GameMap {
    ...
    // New method almost identical to above but returns shortest distance to destination
    public int dijkstraPathDist(Position src, Position dest, Entity entity) {
        // if inputs are invalid, no distance
        if (!nodes.containsKey(src) || !nodes.containsKey(dest))
            return 0;
    ...
    public void updateCircuit() {
        Queue<Conductor> queue = getEntities(Switch.class).stream().filter(s -> s.isActivated())
                .collect(Collectors.toCollection(LinkedList::new));
        List<Wire> inactiveWires = getEntities(Wire.class);
        while (!queue.isEmpty()) {
            queue.poll().getCardinallyAdjacentWires(this).stream().forEach(w -> {
                if (inactiveWires.contains(w)) {
                    inactiveWires.remove(w);
                    queue.add(w);
                }
            });
        }
        inactiveWires.stream().forEach(w -> w.deactivate(this));
        getEntities(Logical.class).stream().forEach(l -> l.updateStatus(this));
    }

...
public class Player extends Entity implements Battleable {
    ...
    public boolean pickUp(GameMap map, Entity item) {
        ...
        if (item instanceof Bomb) {
            ...
            bomb.getSubs().stream().forEach(s -> s.unsubscribe(bomb));
    ...
    public boolean isSceptreHolder() {
        return inventory.getFirst(Sceptre.class) != null;
    }

    public boolean areZombiesPresent(GameMap gameMap) {
        List<ZombieToast> zombieToastList = gameMap.getEntities(ZombieToast.class);
        return !zombieToastList.isEmpty();
    }
}
